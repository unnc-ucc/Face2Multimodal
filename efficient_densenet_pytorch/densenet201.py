...
#
#	U = ((-0.169*R-0.331*G+0.5*B)+128)/255
#
#	V = ((0.5*R-0.419*G-0.081*B)+128)/255
#
#	Y=Y[:, : ,np.newaxis]
#
#	U=U[:, : ,np.newaxis]
...
#	V=V[:, : ,np.newaxis]
#
#	img = np.concatenate((Y,U,V), axis=-1)
#
#	return img
...
def train(model, train_set, valid_set, test_set, save, n_epochs=5,
          batch_size=128, lr=0.01, wd=0.0001, momentum=0.9, seed=None):
    if seed is not None:
...
#############################valid_size
def demo(data='./', save='./output201', depth=100, growth_rate=12, efficient=True, valid_size=None,
         n_epochs=5, batch_size=64, seed=None):
    """
    ...
    model.load_state_dict(torch.load('D:\\Model-Training\\efficient_densenet_pytorch\\output201\\model.dat'))
    # Train the model
